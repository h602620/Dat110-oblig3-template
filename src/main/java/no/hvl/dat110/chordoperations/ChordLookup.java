...
public class ChordLookup {
	...
	public NodeInterface findSuccessor(BigInteger key) throws RemoteException {
		...
		NodeInterface a = Util.getProcessStub(suc.getNodeName(), suc.getPort());
			if( Util.checkInterval(key, node.getNodeID().add(BigInteger.ONE), a.getNodeID()))
				returnBool = true;
			else
				returnBool = false;
	...
	private NodeInterface findHighestPredecessor(BigInteger ID) throws RemoteException {
		...
		for(int i = fingerTable.size()-1; i >= 0; i--){
			...
			if(Util.checkInterval(fingerInterface.getNodeID(), node.getNodeID().add(BigInteger.ONE), ID.subtract(BigInteger.ONE))){
				return finger;
			}
